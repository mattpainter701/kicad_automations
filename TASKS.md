...
### T244. Emit an evidence manifest for generated and analyzed designs (P1, HIGH) 🟡 DONE + R4 REMEDIATION OPEN
...
> **Planner follow-up (code-review of shipped v0.33.0, 2026-07-28 — precedence edge case).** `auto_generate_bypass_caps` (`component_db.py:1772`) still *adds* a `100nF` cap for an IC that has power pins but **no** passive metadata (`recommendation is None`), via a labeled `bounded_fallback`/`heuristic` with a retained calc trace. It's defensible under "labeled heuristic, not datasheet-justified," but it's the closest surviving case to fabricate-a-default-instead-of-withhold. **Planner decision for the next T246 touch:** confirm whether a no-metadata IC bypass should instead emit a `CW-PSV` withhold rather than a bounded default. Not a regression — an intent call on where the `bounded_fallback` floor sits.
...
> **EPIC C REMEDIATION (planner code-review of the landed B1–B5, 2026-07-28) — ✅ ALL CLOSED (`f30547b`/`9f60111`/`63dfcb5`), planner-verified on-disk; the v0.34.0 PR is unblocked.** Each fix recomputes/re-verifies at the consumer and ships a re-read-path regression test that closes its false-green; the five remediation test files pass (79 passed, 2 skipped) on a planner-run. All three were the SAME species: a safety gate trusts a *read-back or self-reported* status instead of recomputing it from source — the exact false-green family as R1/R4 (producer path tested, re-read path not). Copper-emitting, transactional-rollback, override-expiry, provenance, pads-XOR-banner, and part-neutrality contracts all held under reproduction; these are the only breaks found.
> - [x] **C.R1 (MED — fab-gate fail-open) — `manufacturing_readiness.py:103` `from_dict`.** Re-confers the `_FABRICATION_GATE_TOKEN` solely because the JSON says `"state":"fabrication_ready"`, with no evidence re-verification; the gerber gate (`dispatcher.py:4850` `require_export_authorized`) then trusts the state alone. A hand-written/stale `manufacturing_readiness.json` with empty `evidence_ids` authorizes gerber/drill export — the T244.4 evidence gate is bypassed (contract C4 violation; reproduced). **Fix:** the gerber gate must co-load the evidence manifest and re-run `require_manufacturing_evidence` (mirror `delivery_manifest.py:44`, which already does), or `from_dict` must not confer the token without proven evidence. Add a `from_dict`-path test (the guard is only tested via the constructor today). Also fold the LOW: `manufacturing_readiness.py:265` `expires_at.replace` raises `AttributeError` instead of `ReadinessContractError` on `None`/non-str — wrap to fail closed with the contract error.
> - [x] **C.R2 (MED — corroboration bypass) — `identity.py:818` (rides in via the reused T247 path from `pcb_handoff.py:335`).** `evaluate_identity_handoff` trusts the reconciliation's self-reported `source_state`/`missing_coverage` instead of recomputing them from the assertions it is also handed; `validate_identity_reconciliation` only checks the ID matches the caller-chosen payload. A single-source bundle + a hand-built `IdentityReconciliation(source_state="agree", missing_coverage=())` reaches real copper (`pad_count=2`), defeating the "<2 independent sources must BLOCK" clause of contract C2 (reproduced end-to-end). Not reachable via the normal `reconcile_identity_assertions` path. **Fix:** `evaluate_identity_handoff` recomputes `source_state`/`missing_coverage` from the passed assertions (or cross-checks against a fresh `reconcile_identity_assertions(records)`) rather than trusting caller fields. *(Root cause is in the T247 identity module reused verbatim as mandated — the handoff itself is faithful; fix at the source so every T247 consumer is covered.)*
> - [x] **C.R3 (MED — placeholder geometry unrefused) — `pcb_handoff.py:359/672`.** `_preflight_component` records `geometry.source` into the authoritative manifest with no check that it isn't `heuristic`; `footprint_lib.py:115` is cache-first with no invalidation and `:209` returns a 5×5 mm default when a footprint is unresolved. A library instance shared with the placement optimizer can carry a cached heuristic 5×5 that the handoff then stamps into an `authoritative_board` manifest unrefused (contract C4 "refuse placeholder geometry" not enforced; reproduced). Copper is still real, but placeholder dims feed downstream clearance/placement reasoning. **Fix:** `_preflight_component` raises `PcbHandoffError` when `geometry.source == "heuristic"` / `confidence == "heuristic"`; key the geometry cache on resolved path/content-hash so a resolution-state change invalidates it.
