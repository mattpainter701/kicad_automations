...
## Sprint 49 - v0.30.4 Incremental Legacy Migration ✅ DONE
...
**Scope:** T180 + T181, with T182 completed as a stretch goal.
...
Files: `src/circuit_weaver/subcircuits/base.py`, `tests/test_template_structure.py`, `tests/test_template_parity.py` (new if needed)

### T181. Delete specialized topology legacy classes (P1, MEDIUM) ✅ DONE

Depends on: T180 (registry flipped, tests confirm no regressions).

- [x] Write output-parity tests for each topology (34 tests: 24 switching + 10 linear). Fixed two builder gaps: boost inductor net order (vin_net→sw_net), buck_boost inductor net order (l1_net→l2_net) with dual bypass caps and VAUX decoupling, plus PG boundary port and PS_SYNC tied to GND.
- [x] Fix gaps in `topology_builders.py` uncovered by parity tests.
- [x] Remove `buck.py`, `boost.py`, `buck_boost.py`, `ldo.py` from `subcircuits/`. Remove their imports and `reg.register()` calls from `_build_default_registry()`. Update `available_types()` to union registry keys with `list_topologies()`.
- [x] Update test references: `test_template_structure.py`, `test_presentation.py`, `test_sprint3.py` — replace deleted class imports with registry-backed `DataDrivenTemplate` lookups. Update `dispatcher.py` list-templates to mark data-driven source.
- [x] Update `scripts/audit_legacy_templates.py` and `scripts/extract_ic_data.py` to remove deleted imports/calls.
- [x] Confirm full test suite passes (905 pass, 3 pre-existing unrelated failures).
- [x] Confirm 9-archetype corpus `test_corpus_validate_no_hard_errors` (5 pass, 4 pre-existing unrelated failures in non-buck/boost/buck_boost/ldo designs).
- [x] Re-run `I:/my_circuit` validate/generate probe (connector+opamp design only — no buck/boost/buck_boost/ldo needed; validate exits 2 only due to placement_readiness warnings, generate blocked by same pre-existing connectivity issues).

### T182. Port and delete thin verdict-A templates (P2, MEDIUM) — STRETCH ✅ DONE

Depends on: T180, T181.

- [x] Identify exact remaining verdict-A templates from `docs/legacy_template_audit.md`: `can_transceiver`, `eeprom`, `protection` (the other verdict-A templates were deleted in T181).
- [x] Add regression/parity coverage in `tests/test_template_parity_generic_thin.py` for CAN transceiver nets/options, EEPROM I2C/SPI strapping, and passive protection device contract.
- [x] Port legacy behavior into data-driven builders: `build_can_transceiver`, `build_eeprom`, `build_protection`.
- [x] Delete `can_transceiver.py`, `eeprom.py`, `protection.py` from `subcircuits/`. Remove their imports and `reg.register()` calls from `_build_default_registry()`.
- [x] Update tests and scripts that referenced deleted legacy classes/databases.
- [x] Confirm focused tests pass (`9 passed`) and `py -m ruff check src tests` passes.
- [x] Confirm full suite + corpus: full suite is `980 passed, 20 failed, 1 skipped`; corpus hard-error test is `7 passed, 2 failed`. Remaining failures are pre-existing T180 boundary-port/generic-builder gaps and the count is reduced from 26 to 20.

Files: ~7 template files (deleted), `subcircuits/base.py`, `tests/test_template_parity_generic_thin.py` (new)
