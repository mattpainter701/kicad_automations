...
| Placement preview | `{project}_placement.kicad_pcb` | Visual placement hint only; it has no electrical pads/nets and is not routable |
...
Circuit Weaver generates placement guidance, then KiCad creates the electrical PCB. Keep those stages separate: the generated `*_placement.kicad_pcb` is a preview and must not be routed or sent to fabrication.

### 1 — Generate initial placement

`generate` returns the path to `artifact_manifest.json`. Resolve that manifest's portable `root_schematic` and artifact paths relative to the manifest directory instead of guessing filenames. Check `verification_status` before claiming KiCad verification. It also produces a topology-aware placement preview:

```bash
circuit-weaver generate design.yaml -o ./output
# → output/{project}.kicad_sch
# → output/{project}_placement.kicad_pcb   (non-electrical preview)
